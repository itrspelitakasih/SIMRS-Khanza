...
public final class SatuSehatKirimRiskAssessment extends javax.swing.JDialog {
    ...
    // 0=P, 1=No.Rawat, 2=No.RM, 3=Nama Pasien, 4=No.KTP, 5=Nama Dokter, 6=No.KTP Dokter,
    // 7=ID Encounter, 8=Tgl Registrasi, 9=Hasil Skrining, 10=Berjalan A, 11=Berjalan B, 12=Berjalan C,
    // 13=ID RiskAssessment
    public SatuSehatKirimRiskAssessment(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode=new DefaultTableModel(null,new String[]{
                "P","No.Rawat","No.RM","Nama Pasien","No.KTP Pasien","Nama Dokter","No.KTP Dokter",
                "ID Encounter","Tgl Registrasi","Hasil Skrining Jatuh",
                "Tdk Seimbang/Sempoyongan","Jalan Dg Alat Bantu","Menopang Saat Duduk/Berdiri",
                "ID RiskAssessment"
            }){
        ...
        for (i = 0; i < 14; i++) {
            ...
            else if(i==4) column.setPreferredWidth(110);
            else if(i==5) column.setPreferredWidth(150);
            else if(i==6) column.setPreferredWidth(110);
            else if(i==7) column.setPreferredWidth(215);
            else if(i==8) column.setPreferredWidth(130);
            else if(i==9) column.setPreferredWidth(250);
            else if(i==10) column.setPreferredWidth(60);
            else if(i==11) column.setPreferredWidth(60);
            else if(i==12) column.setPreferredWidth(60);
            else if(i==13) column.setPreferredWidth(220);
        }
        ...
        styleSheet.addRule(".isi td{border-right: 1px solid #e2e7dd;font: 8.5px Arial;height:12px;border-bottom: 1px solid #e2e7dd;background: #ffffff;color:#323232;}.isi2 td{font: 8.5px Arial;border:none;height:12px;background: #ffffff;color:#323232;}");
    ...
    private void initComponents() {
        ...
        ppPilihSemua.setBackground(new java.awt.Color(255,255,254)); ppPilihSemua.setFont(new java.awt.Font("Arial",0,11)); ppPilihSemua.setForeground(new java.awt.Color(50,50,50));
        ...
        ppBersihkan.setBackground(new java.awt.Color(255,255,254)); ppBersihkan.setFont(new java.awt.Font("Arial",0,11)); ppBersihkan.setForeground(new java.awt.Color(50,50,50));
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240,245,235)), "::[ Pengiriman Data Risk Assessment Satu Sehat ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial",0,11), new java.awt.Color(50,50,50)));
        internalFrame1.setFont(new java.awt.Font("Arial",0,11)); internalFrame1.setName("internalFrame1"); internalFrame1.setLayout(new java.awt.BorderLayout(1,1));
        ...
        BtnKirim.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/34.png"))); BtnKirim.setMnemonic('K'); BtnKirim.setText("Kirim"); BtnKirim.setToolTipText("Alt+K"); BtnKirim.setName("BtnKirim"); BtnKirim.setPreferredSize(new java.awt.Dimension(100,30));
        BtnKirim.addActionListener(e -> BtnKirimActionPerformed(e)); panelGlass8.add(BtnKirim);
        BtnUpdate.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/edit_f2.png"))); BtnUpdate.setMnemonic('U'); BtnUpdate.setText("Update"); BtnUpdate.setToolTipText("Alt+U"); BtnUpdate.setName("BtnUpdate"); BtnUpdate.setPreferredSize(new java.awt.Dimension(100,30));
        BtnUpdate.addActionListener(e -> BtnUpdateActionPerformed(e)); panelGlass8.add(BtnUpdate);
        ...
        jLabel15.setText("Tgl.Registrasi :"); jLabel15.setName("jLabel15"); jLabel15.setPreferredSize(new java.awt.Dimension(85,23)); panelGlass9.add(jLabel15);
        ...
        jLabel18.setText("Status Kirim:"); jLabel18.setName("jLabel18"); jLabel18.setPreferredSize(new java.awt.Dimension(70,23)); panelGlass9.add(jLabel18);
        cmbStatus.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"Semua","Belum dikirim","Sudah dikirim"})); cmbStatus.setName("cmbStatus"); panelGlass9.add(cmbStatus);
    ...
    private String getRiskLevel(String hasil) {
        String h = hasil.toLowerCase();
        if(h.contains("tinggi")) return "high";
        if(h.contains("rendah")) return "moderate";
        return "low";
    }

    private String getRiskDisplay(String level) {
        if(level.equals("high")) return "High likelihood";
        if(level.equals("moderate")) return "Moderate likelihood";
        return "Low likelihood";
    }

    private String buildRiskAssessmentJson(int row, String idpasien, String iddokter, String existingId) {
        String noRawat=tbObat.getValueAt(row,1).toString();
        String nmPasien=tbObat.getValueAt(row,3).toString();
        String idEncounter=tbObat.getValueAt(row,7).toString();
        String tglReg=tbObat.getValueAt(row,8).toString();
        String hasil=tbObat.getValueAt(row,9).toString();
        String berjalanA=tbObat.getValueAt(row,10).toString();
        String berjalanB=tbObat.getValueAt(row,11).toString();
        String berjalanC=tbObat.getValueAt(row,12).toString();

        String riskLevel=getRiskLevel(hasil);
        String riskDisplay=getRiskDisplay(riskLevel);

        StringBuilder sb = new StringBuilder();
        sb.append("{\"resourceType\":\"RiskAssessment\",");
        if(existingId!=null&&!existingId.equals("")) sb.append("\"id\":\"").append(existingId).append("\",");
        sb.append("\"status\":\"final\",");
        sb.append("\"code\":{\"coding\":[{\"system\":\"http://snomed.info/sct\",\"code\":\"129839007\",\"display\":\"At risk for falls\"}]},");
        ...
        sb.append("\"occurrenceDateTime\":\"").append(tglReg.replaceAll(" ","T")).append("+07:00\",");
        sb.append("\"performer\":{\"reference\":\"Practitioner/").append(iddokter).append("\"},");
        sb.append("\"prediction\":[{");
        sb.append("\"outcome\":{\"text\":\"Risiko Jatuh\"},");
        sb.append("\"qualitativeRisk\":{\"coding\":[{\"system\":\"http://terminology.hl7.org/CodeSystem/risk-probability\",\"code\":\"").append(riskLevel).append("\",\"display\":\"").append(riskDisplay).append("\"}]},");
        sb.append("\"rationale\":\"").append(hasil.replaceAll("\"","'")).append(". ");
        sb.append("Tidak seimbang/sempoyongan: ").append(berjalanA);
        sb.append(", Jalan dengan alat bantu: ").append(berjalanB);
        sb.append(", Menopang saat duduk/berdiri: ").append(berjalanC).append("\"");
        sb.append("}]");
        sb.append("}");
        return sb.toString();
    }
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {
        for(i=0;i<tbObat.getRowCount();i++){
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,4).toString().equals(""))&&(!tbObat.getValueAt(i,6).toString().equals(""))&&(!tbObat.getValueAt(i,7).toString().equals(""))&&tbObat.getValueAt(i,13).toString().equals("")){
                try {
                    ...
                    try{
                        ...
                        json = buildRiskAssessmentJson(i, idpasien, iddokter, null);
                        System.out.println("URL : "+link+"/RiskAssessment");
                        System.out.println("Request JSON : "+json);
                        requestEntity = new HttpEntity(json,headers);
                        json=api.getRest().exchange(link+"/RiskAssessment", HttpMethod.POST, requestEntity, String.class).getBody();
                        System.out.println("Result JSON : "+json);
                        root = mapper.readTree(json); response = root.path("id");
                        if(!response.asText().equals("")){
                            if(Sequel.menyimpantf2("satu_sehat_riskassessment","?,?","Risk Assessment",2,new String[]{
    ...
    private void BtnUpdateActionPerformed(java.awt.event.ActionEvent evt) {
        for(i=0;i<tbObat.getRowCount();i++){
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,4).toString().equals(""))&&(!tbObat.getValueAt(i,6).toString().equals(""))&&(!tbObat.getValueAt(i,13).toString().equals(""))){
                try {
                    idpasien=cekViaSatuSehat.tampilIDPasien(tbObat.getValueAt(i,4).toString());
                    iddokter=cekViaSatuSehat.tampilIDParktisi(tbObat.getValueAt(i,6).toString());
                    try{
                        headers = new HttpHeaders(); headers.setContentType(MediaType.APPLICATION_JSON);
                        headers.add("Authorization", "Bearer "+api.TokenSatuSehat());
                        json = buildRiskAssessmentJson(i, idpasien, iddokter, tbObat.getValueAt(i,13).toString());
                        requestEntity = new HttpEntity(json,headers);
                        json=api.getRest().exchange(link+"/RiskAssessment/"+tbObat.getValueAt(i,13).toString(), HttpMethod.PUT, requestEntity, String.class).getBody();
                        System.out.println("Result JSON : "+json);
                        tbObat.setValueAt(false,i,0);
                    }catch(Exception e){ System.out.println("Notifikasi Bridging : "+e); }
                } catch (Exception e) { System.out.println("Notifikasi : "+e); }
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try{
            ...
            htmlContent.append("<tr class='isi'><td valign='middle' bgcolor='#FFFAFA' align='center'><b>No.Rawat</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>Nama Pasien</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>Hasil Skrining</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>ID</b></td></tr>");
            for (i = 0; i < tabMode.getRowCount(); i++) {
                htmlContent.append("<tr class='isi'><td valign='top'>"+tbObat.getValueAt(i,1)+"</td><td valign='top'>"+tbObat.getValueAt(i,3)+"</td><td valign='top'>"+tbObat.getValueAt(i,9)+"</td><td valign='top'>"+tbObat.getValueAt(i,13)+"</td></tr>");
            }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SatuSehatKirimRiskAssessment dialog = new SatuSehatKirimRiskAssessment(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        ...
        try{
            try {
                ps = koneksi.prepareStatement("CREATE TABLE IF NOT EXISTS satu_sehat_riskassessment (no_rawat varchar(17) NOT NULL, id_riskassessment varchar(50) DEFAULT NULL, PRIMARY KEY (no_rawat)) ENGINE=InnoDB DEFAULT CHARSET=latin1");
                ps.execute(); ps.close();
            } catch (Exception e) {}

            ps=koneksi.prepareStatement(
                   "select reg_periksa.no_rawat,reg_periksa.no_rkm_medis,reg_periksa.tgl_registrasi,reg_periksa.jam_reg,"+
                   "pasien.nm_pasien,pasien.no_ktp,reg_periksa.kd_dokter,pegawai.nama as nm_dokter,pegawai.no_ktp as ktpdokter,"+
                   "satu_sehat_encounter.id_encounter,"+
                   "penilaian_awal_keperawatan_ralan.hasil,penilaian_awal_keperawatan_ralan.berjalan_a,"+
                   "penilaian_awal_keperawatan_ralan.berjalan_b,penilaian_awal_keperawatan_ralan.berjalan_c,"+
                   "ifnull(satu_sehat_riskassessment.id_riskassessment,'') as id_riskassessment "+
                   ...
                   "inner join penilaian_awal_keperawatan_ralan on penilaian_awal_keperawatan_ralan.no_rawat=reg_periksa.no_rawat "+
                   "left join satu_sehat_riskassessment on satu_sehat_riskassessment.no_rawat=reg_periksa.no_rawat "+
                   "where reg_periksa.tgl_registrasi between ? and ? "+
                   (cmbStatus.getSelectedItem().toString().equals("Belum dikirim")?"and (satu_sehat_riskassessment.id_riskassessment is null or satu_sehat_riskassessment.id_riskassessment='') ":"")+
                   (cmbStatus.getSelectedItem().toString().equals("Sudah dikirim")?"and satu_sehat_riskassessment.id_riskassessment is not null and satu_sehat_riskassessment.id_riskassessment!='' ":"")+
                   (TCari.getText().equals("")?"":"and (reg_periksa.no_rawat like ? or pasien.nm_pasien like ? or pasien.no_ktp like ? or penilaian_awal_keperawatan_ralan.hasil like ?) "));
            try {
                ps.setString(1,Valid.SetTgl(DTPCari1.getSelectedItem()+""));
                ps.setString(2,Valid.SetTgl(DTPCari2.getSelectedItem()+""));
                if(!TCari.getText().equals("")){
                    ps.setString(3,"%"+TCari.getText()+"%"); ps.setString(4,"%"+TCari.getText()+"%");
                    ps.setString(5,"%"+TCari.getText()+"%"); ps.setString(6,"%"+TCari.getText()+"%");
                }
                rs=ps.executeQuery();
                while(rs.next()){
                    tabMode.addRow(new Object[]{
                        false,rs.getString("no_rawat"),rs.getString("no_rkm_medis"),rs.getString("nm_pasien"),rs.getString("no_ktp"),
                        rs.getString("nm_dokter"),rs.getString("ktpdokter"),rs.getString("id_encounter"),
                        rs.getString("tgl_registrasi")+" "+rs.getString("jam_reg"),
                        rs.getString("hasil"),rs.getString("berjalan_a"),rs.getString("berjalan_b"),rs.getString("berjalan_c"),
                        rs.getString("id_riskassessment")
                    });
                }
