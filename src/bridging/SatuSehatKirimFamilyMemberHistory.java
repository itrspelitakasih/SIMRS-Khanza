...
public final class SatuSehatKirimFamilyMemberHistory extends javax.swing.JDialog {
    ...
    // 7=ID Encounter, 8=Tgl Registrasi, 9=Riwayat Penyakit Keluarga, 10=ID FamilyMemberHistory
    public SatuSehatKirimFamilyMemberHistory(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode=new DefaultTableModel(null,new String[]{
                ...
                "ID Encounter","Tgl Registrasi","Riwayat Penyakit Keluarga","ID FamilyMemberHistory"
            }){
        ...
        for (i = 0; i < 11; i++) {
            ...
            else if(i==10) column.setPreferredWidth(220);
        }
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240,245,235)), "::[ Pengiriman Data Family Member History Satu Sehat ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial",0,11), new java.awt.Color(50,50,50)));
    ...
    private String buildFamilyMemberHistoryJson(int row, String idpasien, String existingId) {
        String rpk = tbObat.getValueAt(row,9).toString().replaceAll("\"","'");
        StringBuilder sb = new StringBuilder();
        sb.append("{\"resourceType\":\"FamilyMemberHistory\",");
        if(existingId!=null&&!existingId.equals("")) sb.append("\"id\":\"").append(existingId).append("\",");
        sb.append("\"status\":\"completed\",");
        sb.append("\"patient\":{\"reference\":\"Patient/").append(idpasien).append("\",\"display\":\"").append(tbObat.getValueAt(row,3).toString()).append("\"},");
        sb.append("\"relationship\":{\"coding\":[{\"system\":\"http://terminology.hl7.org/CodeSystem/v3-RoleCode\",\"code\":\"FAMMEMB\",\"display\":\"family member\"}]},");
        sb.append("\"condition\":[{\"code\":{\"text\":\"").append(rpk).append("\"}}]");
        sb.append("}");
        return sb.toString();
    }
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {
        for(i=0;i<tbObat.getRowCount();i++){
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,4).toString().equals(""))&&(!tbObat.getValueAt(i,7).toString().equals(""))&&tbObat.getValueAt(i,10).toString().equals("")&&(!tbObat.getValueAt(i,9).toString().equals(""))&&(!tbObat.getValueAt(i,9).toString().equals("-"))){
                try {
                    ...
                    try{
                        ...
                        json = buildFamilyMemberHistoryJson(i, idpasien, null);
                        System.out.println("URL : "+link+"/FamilyMemberHistory");
                        System.out.println("Request JSON : "+json);
                        requestEntity = new HttpEntity(json,headers);
                        json=api.getRest().exchange(link+"/FamilyMemberHistory", HttpMethod.POST, requestEntity, String.class).getBody();
                        System.out.println("Result JSON : "+json);
                        root = mapper.readTree(json); response = root.path("id");
                        if(!response.asText().equals("")){
                            if(Sequel.menyimpantf2("satu_sehat_familymemberhistory","?,?","Family History",2,new String[]{
                                tbObat.getValueAt(i,1).toString(),response.asText()
                            })==true){
                                tbObat.setValueAt(response.asText(),i,10);
    ...
    private void BtnUpdateActionPerformed(java.awt.event.ActionEvent evt) {
        for(i=0;i<tbObat.getRowCount();i++){
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,4).toString().equals(""))&&(!tbObat.getValueAt(i,10).toString().equals(""))){
                try {
                    idpasien=cekViaSatuSehat.tampilIDPasien(tbObat.getValueAt(i,4).toString());
                    try{
                        headers = new HttpHeaders(); headers.setContentType(MediaType.APPLICATION_JSON);
                        headers.add("Authorization", "Bearer "+api.TokenSatuSehat());
                        json = buildFamilyMemberHistoryJson(i, idpasien, tbObat.getValueAt(i,10).toString());
                        requestEntity = new HttpEntity(json,headers);
                        json=api.getRest().exchange(link+"/FamilyMemberHistory/"+tbObat.getValueAt(i,10).toString(), HttpMethod.PUT, requestEntity, String.class).getBody();
                        System.out.println("Result JSON : "+json);
                        tbObat.setValueAt(false,i,0);
                    }catch(Exception e){ System.out.println("Notifikasi Bridging : "+e); }
                } catch (Exception e) { System.out.println("Notifikasi : "+e); }
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try{
            ...
            htmlContent.append("<tr class='isi'><td valign='middle' bgcolor='#FFFAFA' align='center'><b>No.Rawat</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>Nama Pasien</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>Riwayat Penyakit Keluarga</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>ID</b></td></tr>");
            for (i = 0; i < tabMode.getRowCount(); i++) {
                htmlContent.append("<tr class='isi'><td valign='top'>"+tbObat.getValueAt(i,1)+"</td><td valign='top'>"+tbObat.getValueAt(i,3)+"</td><td valign='top'>"+tbObat.getValueAt(i,9)+"</td><td valign='top'>"+tbObat.getValueAt(i,10)+"</td></tr>");
            }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SatuSehatKirimFamilyMemberHistory dialog = new SatuSehatKirimFamilyMemberHistory(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        ...
        try{
            try {
                ps = koneksi.prepareStatement("CREATE TABLE IF NOT EXISTS satu_sehat_familymemberhistory (no_rawat varchar(17) NOT NULL, id_familymemberhistory varchar(50) DEFAULT NULL, PRIMARY KEY (no_rawat)) ENGINE=InnoDB DEFAULT CHARSET=latin1");
                ps.execute(); ps.close();
            } catch (Exception e) {}
            ...
            ps=koneksi.prepareStatement(
                   ...
                   "satu_sehat_encounter.id_encounter,penilaian_awal_keperawatan_ralan.rpk,"+
                   "ifnull(satu_sehat_familymemberhistory.id_familymemberhistory,'') as id_familymemberhistory "+
                   ...
                   "left join satu_sehat_familymemberhistory on satu_sehat_familymemberhistory.no_rawat=reg_periksa.no_rawat "+
                   "where penilaian_awal_keperawatan_ralan.rpk<>'' and penilaian_awal_keperawatan_ralan.rpk<>'-' "+
                   "and reg_periksa.tgl_registrasi between ? and ? "+
                   (cmbStatus.getSelectedItem().toString().equals("Belum dikirim")?"and (satu_sehat_familymemberhistory.id_familymemberhistory is null or satu_sehat_familymemberhistory.id_familymemberhistory='') ":"")+
                   (cmbStatus.getSelectedItem().toString().equals("Sudah dikirim")?"and satu_sehat_familymemberhistory.id_familymemberhistory is not null and satu_sehat_familymemberhistory.id_familymemberhistory!='' ":"")+
                   (TCari.getText().equals("")?"":"and (reg_periksa.no_rawat like ? or pasien.nm_pasien like ? or pasien.no_ktp like ? or penilaian_awal_keperawatan_ralan.rpk like ?) "));
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("rpk"),rs.getString("id_familymemberhistory")
                    });
                }
