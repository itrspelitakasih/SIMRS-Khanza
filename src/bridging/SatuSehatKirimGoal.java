...
public final class SatuSehatKirimGoal extends javax.swing.JDialog {
    ...
    // 7=ID Encounter, 8=Tgl Registrasi, 9=Masalah Keperawatan, 10=Rencana Keperawatan, 11=ID Goal
    public SatuSehatKirimGoal(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode=new DefaultTableModel(null,new String[]{
                ...
                "ID Encounter","Tgl Registrasi","Masalah Keperawatan","Rencana Keperawatan","ID Goal"
            }){
        ...
        for (i = 0; i < 12; i++) {
            ...
            else if(i==9) column.setPreferredWidth(200);
            else if(i==10) column.setPreferredWidth(250);
            else if(i==11) column.setPreferredWidth(220);
        }
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240,245,235)), "::[ Pengiriman Data Goal Satu Sehat ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial",0,11), new java.awt.Color(50,50,50)));
    ...
    private String buildGoalJson(int row, String idpasien, String existingId) {
        ...
        String masalah=tbObat.getValueAt(row,9).toString().replaceAll("\"","'");
        String rencana=tbObat.getValueAt(row,10).toString().replaceAll("\"","'");

        StringBuilder sb = new StringBuilder();
        sb.append("{\"resourceType\":\"Goal\",");
        if(existingId!=null&&!existingId.equals("")) sb.append("\"id\":\"").append(existingId).append("\",");
        sb.append("\"lifecycleStatus\":\"active\",");
        sb.append("\"description\":{\"text\":\"").append(rencana).append("\"},");
        sb.append("\"subject\":{\"reference\":\"Patient/").append(idpasien).append("\",\"display\":\"").append(nmPasien).append("\"},");
        sb.append("\"startDate\":\"").append(tglReg.substring(0,10)).append("\",");
        // addresses: masalah keperawatan
        if(!masalah.equals("")){
            sb.append("\"addresses\":[{\"display\":\"").append(masalah).append("\"}],");
        }
        // note
        sb.append("\"note\":[{\"text\":\"Masalah: ").append(masalah).append(". Rencana: ").append(rencana).append("\"}]");
        sb.append("}");
        return sb.toString();
    }
    ...
    private void BtnKirimActionPerformed(java.awt.event.ActionEvent evt) {
        for(i=0;i<tbObat.getRowCount();i++){
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,4).toString().equals(""))&&(!tbObat.getValueAt(i,7).toString().equals(""))&&tbObat.getValueAt(i,11).toString().equals("")){
                try {
                    ...
                    try{
                        ...
                        json = buildGoalJson(i, idpasien, null);
                        System.out.println("URL : "+link+"/Goal");
                        System.out.println("Request JSON : "+json);
                        requestEntity = new HttpEntity(json,headers);
                        json=api.getRest().exchange(link+"/Goal", HttpMethod.POST, requestEntity, String.class).getBody();
                        System.out.println("Result JSON : "+json);
                        root = mapper.readTree(json); response = root.path("id");
                        if(!response.asText().equals("")){
                            if(Sequel.menyimpantf2("satu_sehat_goal","?,?","Goal",2,new String[]{
    ...
    private void BtnUpdateActionPerformed(java.awt.event.ActionEvent evt) {
        for(i=0;i<tbObat.getRowCount();i++){
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,4).toString().equals(""))&&(!tbObat.getValueAt(i,11).toString().equals(""))){
                try {
                    idpasien=cekViaSatuSehat.tampilIDPasien(tbObat.getValueAt(i,4).toString());
                    try{
                        headers = new HttpHeaders(); headers.setContentType(MediaType.APPLICATION_JSON);
                        headers.add("Authorization", "Bearer "+api.TokenSatuSehat());
                        json = buildGoalJson(i, idpasien, tbObat.getValueAt(i,11).toString());
                        requestEntity = new HttpEntity(json,headers);
                        json=api.getRest().exchange(link+"/Goal/"+tbObat.getValueAt(i,11).toString(), HttpMethod.PUT, requestEntity, String.class).getBody();
                        System.out.println("Result JSON : "+json);
                        tbObat.setValueAt(false,i,0);
                    }catch(Exception e){ System.out.println("Notifikasi Bridging : "+e); }
                } catch (Exception e) { System.out.println("Notifikasi : "+e); }
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        try{
            ...
            htmlContent.append("<tr class='isi'><td valign='middle' bgcolor='#FFFAFA' align='center'><b>No.Rawat</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>Nama Pasien</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>Masalah</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>Rencana</b></td><td valign='middle' bgcolor='#FFFAFA' align='center'><b>ID</b></td></tr>");
            for (i = 0; i < tabMode.getRowCount(); i++) {
                htmlContent.append("<tr class='isi'><td valign='top'>"+tbObat.getValueAt(i,1)+"</td><td valign='top'>"+tbObat.getValueAt(i,3)+"</td><td valign='top'>"+tbObat.getValueAt(i,9)+"</td><td valign='top'>"+tbObat.getValueAt(i,10)+"</td><td valign='top'>"+tbObat.getValueAt(i,11)+"</td></tr>");
            }
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            SatuSehatKirimGoal dialog = new SatuSehatKirimGoal(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        ...
        try{
            try {
                ps = koneksi.prepareStatement("CREATE TABLE IF NOT EXISTS satu_sehat_goal (no_rawat varchar(17) NOT NULL, id_goal varchar(50) DEFAULT NULL, PRIMARY KEY (no_rawat)) ENGINE=InnoDB DEFAULT CHARSET=latin1");
                ps.execute(); ps.close();
            } catch (Exception e) {}
            ...
            ps=koneksi.prepareStatement(
                   ...
                   "group_concat(distinct master_masalah_keperawatan.nama_masalah separator ', ') as masalah,"+
                   "penilaian_awal_keperawatan_ralan.rencana,"+
                   "ifnull(satu_sehat_goal.id_goal,'') as id_goal "+
                   ...
                   "left join penilaian_awal_keperawatan_ralan_masalah on penilaian_awal_keperawatan_ralan_masalah.no_rawat=reg_periksa.no_rawat "+
                   "left join master_masalah_keperawatan on master_masalah_keperawatan.kode_masalah=penilaian_awal_keperawatan_ralan_masalah.kode_masalah "+
                   "left join satu_sehat_goal on satu_sehat_goal.no_rawat=reg_periksa.no_rawat "+
                   "where penilaian_awal_keperawatan_ralan.rencana<>'' "+
                   "and reg_periksa.tgl_registrasi between ? and ? "+
                   (cmbStatus.getSelectedItem().toString().equals("Belum dikirim")?"and (satu_sehat_goal.id_goal is null or satu_sehat_goal.id_goal='') ":"")+
                   (cmbStatus.getSelectedItem().toString().equals("Sudah dikirim")?"and satu_sehat_goal.id_goal is not null and satu_sehat_goal.id_goal!='' ":"")+
                   (TCari.getText().equals("")?"":"and (reg_periksa.no_rawat like ? or pasien.nm_pasien like ? or pasien.no_ktp like ? or penilaian_awal_keperawatan_ralan.rencana like ?) ")+
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("masalah")==null?"":rs.getString("masalah"),
                        rs.getString("rencana"),rs.getString("id_goal")
                    });
                }
