 ...
 * @author perpustakaan
 */
public final class RMHasilPemeriksaanUSGThyroid extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public RMHasilPemeriksaanUSGThyroid(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        templatehasil.addWindowListener(new WindowListener() {
            ...
            @Override
            public void windowClosed(WindowEvent e) {
                if(akses.getform().equals("RMHasilPemeriksaanUSGThyroid")){
    ...
    private void initComponents() {
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Hasil Pemeriksaan USG Thyroid ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial", 0, 12), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        Tanggal.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "23-01-2025 21:26:39" }));
    ...
    private void MnPenilaianMedisActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnPenilaianMedisActionPerformed
        if(tbObat.getSelectedRow()>-1){
            ...
            param.put("photo","http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/hasilpemeriksaanusgthyroid/"+lokasifile);
            ...
            param.put("tanggal",Sequel.cariIsi("select tanggal from hasil_pemeriksaan_usg_thyroid where no_rawat=?",TNoRw.getText()));
            Valid.MyReportqry("rptCetakHasilPemeriksaanUSGThyroid.jasper","report","::[ Formulir Hasil Pemeriksaan USG ]::",
                "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,hasil_pemeriksaan_usg_thyroid.tanggal,"+
                "hasil_pemeriksaan_usg_thyroid.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_usg_thyroid.diagnosa_klinis,hasil_pemeriksaan_usg_thyroid.kiriman_dari,"+
                "hasil_pemeriksaan_usg_thyroid.hasil,"+
                "hasil_pemeriksaan_usg_thyroid.kesimpulan from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                "inner join hasil_pemeriksaan_usg_thyroid on reg_periksa.no_rawat=hasil_pemeriksaan_usg_thyroid.no_rawat "+
                "inner join dokter on hasil_pemeriksaan_usg_thyroid.kd_dokter=dokter.kd_dokter where hasil_pemeriksaan_usg_thyroid.no_rawat='"+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString()+"'",param);
        }
    ...
    private void btnAmbilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            if(tbObat.getSelectedRow()>-1){
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Valid.panggilUrl("hasilpemeriksaanusgthyroid/login.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&no_rawat="+tbObat.getValueAt(tbObat.getSelectedRow(),0).toString());
                this.setCursor(Cursor.getDefaultCursor()); 
            }else{
...
//        Valid.pindah2(evt,Doppler,BtnSimpan);
    ...
    private void btnAmbilPhotoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilPhotoActionPerformed
        akses.setform("RMHasilPemeriksaanUSGThyroid");
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            RMHasilPemeriksaanUSGThyroid dialog = new RMHasilPemeriksaanUSGThyroid(new javax.swing.JFrame(), true);
        ...
        try{
            if(TCari.getText().trim().equals("")){
                ps=koneksi.prepareStatement(
                        ...
                        "hasil_pemeriksaan_usg_thyroid.kesimpulan from reg_periksa inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                        "inner join hasil_pemeriksaan_usg_thyroid on reg_periksa.no_rawat=hasil_pemeriksaan_usg_thyroid.no_rawat "+
                        "inner join dokter on hasil_pemeriksaan_usg_thyroid.kd_dokter=dokter.kd_dokter where "+
                        "hasil_pemeriksaan_usg_thyroid.tanggal between ? and ? order by hasil_pemeriksaan_usg_thyroid.tanggal");
            }else{
                ps=koneksi.prepareStatement(
                        "select reg_periksa.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir,hasil_pemeriksaan_usg_thyroid.tanggal,"+
                        "hasil_pemeriksaan_usg_thyroid.kd_dokter,dokter.nm_dokter,hasil_pemeriksaan_usg_thyroid.diagnosa_klinis,hasil_pemeriksaan_usg_thyroid.kiriman_dari,"+
                        ...
                        "inner join hasil_pemeriksaan_usg_thyroid on reg_periksa.no_rawat=hasil_pemeriksaan_usg_thyroid.no_rawat "+
                        "inner join dokter on hasil_pemeriksaan_usg_thyroid.kd_dokter=dokter.kd_dokter where "+
                        "hasil_pemeriksaan_usg_thyroid.tanggal between ? and ? and (reg_periksa.no_rawat like ? or pasien.no_rkm_medis like ? or pasien.nm_pasien like ? or "+
                        "hasil_pemeriksaan_usg_thyroid.kd_dokter like ? or dokter.nm_dokter like ?) order by hasil_pemeriksaan_usg_thyroid.tanggal");
            }
    ...
    private void hapus() {
        if(Sequel.queryu2tf("delete from hasil_pemeriksaan_usg_thyroid where no_rawat=?",1,new String[]{
...
//    private void ganti() {
//        if(Sequel.mengedittf("hasil_pemeriksaan_usg_thyroid","no_rawat=?","no_rawat=?,tanggal=?,kd_dokter=?,diagnosa_klinis=?,kiriman_dari=?,hasil=?,kesimpulan=?",8,new String[]{
...
//    }
    ...
    // SQL update query
    String sql = "UPDATE hasil_pemeriksaan_usg_thyroid SET no_rawat=?, tanggal=?, kd_dokter=?, diagnosa_klinis=?, kiriman_dari=?, hasil=?, kesimpulan=? WHERE no_rawat=?";
    ...
    private void panggilPhoto() {
        if(FormPhoto.isVisible()==true){
            lokasifile="";
            try {
                ps=koneksi.prepareStatement("select hasil_pemeriksaan_usg_thyroid_gambar.photo from hasil_pemeriksaan_usg_thyroid_gambar where hasil_pemeriksaan_usg_thyroid_gambar.no_rawat=?");
                try {
                    ...
                    if(rs.next()){
                        if(rs.getString("photo").equals("")||rs.getString("photo").equals("-")){
                            ...
                            LoadHTML2.setText("<html><body><center><a href='http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/hasilpemeriksaanusgthyroid/"+rs.getString("photo")+"'><img src='http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/hasilpemeriksaanusgthyroid/"+rs.getString("photo")+"' alt='photo' width='550' height='550'/></a></center></body></html>");
                        }  
                    }else{
                        lokasifile="";
                        LoadHTML2.setText("<html><body><center><br><br><font face='Arial' size='2' color='#434343'>Kosong</font></center></body></html>");
    ...
    private void simpan() {
        if(Sequel.menyimpantf("hasil_pemeriksaan_usg_thyroid","?,?,?,?,?,?,?","No.Rawat",7,new String[]{
